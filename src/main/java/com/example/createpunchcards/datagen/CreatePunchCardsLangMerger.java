 ...
 * {@code assets/createpunchcards/lang/default/*.json}, and only the translation <em>keys</em>
 * appear in code (see {@link com.example.createpunchcards.Lang}).
 *
 ...
 * into that same provider via {@link CreatePunchCards#registerLangPartials()}, so the partials
 ...
 */
public class CreatePunchCardsLangMerger {

    private static final Gson GSON = new Gson();

    /** Partial file names under {@code assets/createpunchcards/lang/default/}, without ".json". */
    ...
    public static void mergeInto(BiConsumer<String, String> consumer) {
        for (String partial : PARTIALS) {
            String path = "assets/" + CreatePunchCards.ID + "/lang/default/" + partial + ".json";
            try (InputStream in = CreatePunchCardsLangMerger.class.getClassLoader().getResourceAsStream(path)) {
                if (in == null) {
                    CreatePunchCards.LOGGER.error("Lang partial not found on the classpath: {}", path);
                    continue;
                }
